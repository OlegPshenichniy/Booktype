...
from booktype.apps.convert.plugin import ConversionPlugin
...
class ImageEditorConversionPlugin(ConversionPlugin):

    def __init__(self, convert):
        super(ImageEditorConversionPlugin, self).__init__(convert)

        self._original_book = None
        self._book = None
        self._cache_subdirectory = self.convert.config.get("project_id")
    ...
    def fix_content(self, content):
        ...
        for img_element in content.iter('img'):
    ...
    def _edit_image(self, elem):
        ...
        # proportionally resize according to EPUB_DOCUMENT_WIDTH
        quotient = float(EDITOR_WIDTH) / float(EPUB_DOCUMENT_WIDTH)
        ...
        width_percent = 100 - (100 - 100 * float(transform_data['frameWidth']) / EPUB_DOCUMENT_WIDTH)
