...
from .constants import EPUB_DOCUMENT_WIDTH, EDITOR_WIDTH
        ...
        # create ebooklib image item
            ...
            if os.path.isfile(edited_image_path):
                ...
                with Image.open(edited_image_path) as im:
                    ...
                    _name[1] = _fname + '_edit_' + str(self._ebooklib_item_image_id) + '.' + extension
                    new_ebooklib_item_image.file_name = u'/'.join(_name)

                    # # create unique name
                    # _name = ebooklib_item_image.file_name.rsplit('/', 1)
                    # _fname = _name[1].rsplit('.', 1)[0]
                    # _name[1] = _fname + '_edit_' + str(self._ebooklib_item_image_id) + '.' + extension
                    # new_ebooklib_item_image.file_name = u'/'.join(_name)
