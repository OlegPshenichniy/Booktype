  define(['aloha', 'aloha/plugin', 'jquery',  'jquery19', 'ui/ui', 'aloha/ephemera', 'block/block', 'block/blockmanager',
          'underscore', 'PubSub', 'booktype', 'toolbar/toolbar-plugin', 'simpletable/square-selector'],
      function(Aloha, Plugin, jQuery,  jQuery19, UI, Ephemera, block, BlockManager, _, PubSub, booktype, toolbar, squareSelector) {
        var n = 0;
   ...
              BlockManager.registerBlockType('SimpleTableBlock', SimpleTableBlock);
              ...
              Aloha.bind('aloha-editable-created', function($event, editable) {
                ...
                if(editable.obj && editable.obj.attr('id') == 'contenteditor') {
                  ...
                  // create table
                  tableSizeSelector = new squareSelector.squareSelector(jQuery19('#table-size-selector', $dropdown));
                  tableSizeSelector.draw(8, 8);
                  tableSizeSelector.highlightArea(2, 2);
                  tableSizeSelector.onSelect = function (item, cols, rows) {
                    ...
                    var $table = jQuery('<div class="aloha-table"><table><tbody>' + html + '</tbody></table></div>');
                    $table.alohaBlock({'aloha-block-type': 'SimpleTableBlock'});

                    var limit = Aloha.activeEditable;
                    if (!limit) {
                      limit = jQuery('#contenteditor.aloha-editable');
                    }

                    GENTICS.Utils.Dom.insertIntoDOM($table, range, limit);

                    // close btn-group menu
                    jQuery('button.table-dropdown').closest('div.btn-group.open').removeClass('open');
              ...
              PubSub.sub('aloha.selection.context-change', function(m) {
                ...
                if($td.length > 0) {
                  ...
                  $dropdown.find('.table-size-selector').css('display', 'none');
                  ...
                  $dropdown.find('.table-size-selector').css('display', '');
