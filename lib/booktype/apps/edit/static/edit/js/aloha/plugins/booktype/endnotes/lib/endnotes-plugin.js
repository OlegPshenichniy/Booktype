  ...
  function (Aloha, Plugin, jQuery, jQuery19, UI, Ephemera, block, BlockManager, _, EndnotesBlock, toolbar, PubSub) {
    ...
    return Plugin.create('endnotes', {
      defaultSettings: {
        ...
        disabledMenus: ['endnote-insert', 'insertImage', 'horizontalline', 'pbreak', 'split-chapter']
      },
      init: function () {
        var editable, range;
        var $this = this;
        ...
        PubSub.sub('aloha.selection.context-change', function (message) {
          ...
          // disable insert endnote link if cursor inside endnotes block or in the forbidden tag
           if (jQuery(message.range.endContainer).closest('div.endnoteText').length) {
             _.each($this.settings.disabledMenus, function (menu) { toolbar.disableMenu(menu); });
           } else {
             ...
             _.each($this.settings.disabledMenus, function (menu) { toolbar.enableMenu(menu); });
           }
        });
