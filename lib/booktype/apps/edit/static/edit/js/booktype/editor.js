...
(function (win, jquery, _) {
  ...
  win.booktype.editor = (function () {
    ...
    var DEFAULTS = {
      ...
      config: {
        'global': {
          'tabs': ['online-users', 'chat', 'notes', 'video']
        },
        ...
        function (dta) {
          ...
          activePanel.hide(function () {
            ...
            var _sendNotification = function () {
                if (currentEdit !== null) {
                  win.booktype.sendToCurrentBook({"command": "book_notification",
                    "chapterID": id},
                    function (data) {
    ...
    var _initUI = function () {
      ...
      //VIDEO TAB
      if (isEnabledTab('global', 'video')) {
        var t4 = createLeftTab('video-tab', 'big-icon-history');

        t4.draw = function () {
          // add handle for checkbox
          jquery('#video-enabled').click(function () {
            var videoEnabled = jQuery(this).is(':checked');

            win.booktype.sendToChannel('/video/' + win.booktype.currentBookID + '/', {
                'command': 'video_settings',
        ...
        t4.receivedInvitation = function (inviter, nodeLink) {
          // closed tab notification
          var isVideoOpen = jquery('#video-tab').parent().hasClass('active');
          if (!isVideoOpen) {
            jquery('#video-tab').append('<span class="label label-success"><span class="icon-facetime-video"></span></span>');
          }
          // remove default message
          jquery('#video-invitations > p').remove();

          //if invitation from this user is allready exist
          if (jquery('#username-invitation-' + inviter).length) {
            return;
          }

          // add notification to active section
          var template = _.template(jquery('#templateVideoInvitation').html());

          jquery('.video-invitations').append(template({
            node_link: nodeLink,
            inviter: inviter
          }));
        };
    ...
    var _addVideoUser = function (user) {
      // exclude current user, because it's impossible to call myself
      if (win.booktype.username !== user.username) {
        var $container = jquery('section[source_id=video-tab] ul.video-available-users');
        var template = _.template(jquery('#templateVideoUser').html());
        var gravatar = getGravatar(user.email);
        var fullname = user.first_name + ' ' + user.last_name;

        //Find if the user is already there
        jquery('#username-video-' + user.username).remove();
        $container.append(template({
          username: user.username,
          current_username: win.booktype.username,
          fullname: fullname,
          user_role: 'editor',
          gravatar: gravatar,
          book: win.booktype.currentBookID
        }));
      }
    };

    var _initVideoCall = function (video_users) {
      //set value for settings checkbox
      for (var i in video_users) {
        // if current user is in available for video call users -> set checkbox
        if (win.booktype.username === video_users[i].username) {
          jQuery('#video-enabled').attr('checked', 'checked');
          break;
        }
      }
      // fill available for video users
      jquery.map(video_users, _addVideoUser);
    };
    ...
    var _loadInitialData = function () {
      ...
      win.booktype.sendToCurrentBook({'command': 'init_editor'},
        function (dta) {
          ...
          // initialize video call section
          if (isEnabledTab('global', 'video')) {
            _initVideoCall(dta.videoUsers);
          }
    ...
    var _initEditor = function (settings) {
      ...
      // subscribe to video channel if it is enabled
      if (isEnabledTab('global', 'video')) {
        win.booktype.subscribeToChannel('/video/' + win.booktype.currentBookID + '/',
          function (message) {
            switch (message.command) {

              case 'video_settings_changed':
                // TODO remove it. it's for test
                console.log('Video settings changed');
                break;

              case 'video_retrieve_users':
                // TODO remove logging
                console.log('video_retrieve_users', message.videoUsers);
                // clear video users container
                jquery('section[source_id=video-tab] ul').empty();
                // fill it again with updated data
                jquery.map(message.videoUsers, _addVideoUser);
                break;

              case 'video_invite':
                if (message.invited === win.booktype.username) {
                  tabs[3].receivedInvitation(message.inviter, message.node_link);
                }
                break;
            }
          }
